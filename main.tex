...
\textcolor{cyan}{These examples show that the pseudo-partitive and low-counting contexts may share the same surface form \textit{broskv-e}, while the high-counting context selects a distinct genitive plural form \textit{broskv-í}.}
...
In (\ref{zoha:ex:aab}), we observe that the pseudo-partitive context (\ref{zoha:ex:aabpart}) and the low-counting context (\ref{zoha:ex:aablow}) both select the form \textit{broskv-e}. In (\ref{zoha:ex:aabpart}), it expresses the genitive of singular, while in (\ref{zoha:ex:aablow}) the nominative of plural. In the high-counting context, (\ref{zoha:ex:aabhigh}), the noun selects a different form, \textit{broskv-í}, expressing the genitive of plural.
...
Finally, the ABC pattern is shown in Ukrainian in (\ref{zoha:ex:abc}), where all three contexts are morphologically distinct. The pseudo-partitive form \textit{persyk-a} surfaces in the genitive singular, the low-counting form \textit{persyk-y} shows nominative plural, and the high-counting form \textit{persyk-iv} appears in the genitive plural. The existence of such tripartite distinctions supports the hypothesis that each phrase type introduces an independent structural layer.
...
The root \textit{persyk} lexicalizes up to \textsc{masc}, and since the noun is masculine, the projection \textsc{fem} remains unrealized (black cells). Each quantity context is realized by a distinct morphological suffix, yielding a three-way contrast.
...
Now, however, the structure of \textit{-e} begins in \textsc{fem} and ends in \textsc{mass} for pseudo-partitive and in \textsc{k1} skipping \textsc{quant} for low-counting contexts. For high-counting phrases, the all features are lexicalized.
...
We now turn to the ABB pattern, exemplified by Bulgarian. As shown in Table~\ref{zoha:tab:lexABBcase}, the stem \textit{praskov-} ‘peach’ appears with three forms across the relevant contexts. The pseudo-partitive context uses the suffix \textit{-a}, while both low- and high-counting contexts use the shared suffix \textit{-i}, producing an ABB pattern.
